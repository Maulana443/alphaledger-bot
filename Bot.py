# ===================================================================
# AlphaLedgerFX Bot - FINAL (GC=F + Info Harga)
# ===================================================================
...
def resolve_symbol(user_input: str) -> tuple:
    ...
    if s in ("XAUUSD", "XAUU", "GOLD", "EMAS"):
        return "GC=F", "GLD", 10.0   # 1 GLD = 0.1 ons emas
...
def get_price(primary: str, fallback: str = None, multiplier: float = 1.0):
    # Coba data utama dulu, tanpa multiplier
    ...
    # Data utama gagal, pakai fallback dengan multiplier
    if fallback:
        data = yf.download(fallback, period="1d", progress=False)
        if not data.empty:
            return float(data['Close'].values.flat[-1]) * multiplier
        try:
            tick = yf.Ticker(fallback)
            hist = tick.history(period="1d")
            if not hist.empty:
                return float(hist['Close'].iloc[-1]) * multiplier
        except: pass
    return None
...
async def upgrade_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if is_premium(update.effective_user.id):
        ...
        return
    left = 10 - promo_count()
    kb = [[InlineKeyboardButton("🔥 Bulanan (Rp20.000)" if left>0 else "📅 Bulanan (Rp35.000)", callback_data="pilih_bulanan")],[InlineKeyboardButton("💎 Selamanya (Rp35.000)" if left>0 else "👑 Selamanya (Rp50.000)", callback_data="pilih_selamanya")]]
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for a in angka:
            try:
                bersih = a.replace('.','').replace(',','.')
                nilai = float(bersih)
                if 10000 <= nilai <= 100000:
                    ditemukan.append(int(nilai))
            except: continue
        if ditemukan:
            nominal_max = max(ditemukan)
            if nominal_max == harga_harapan:
                ...
                await context.bot.send_message(
                    ...
                    f"Username: @{uname}\n"
                    f"ID: {uid}\n"
                    f"Paket: {'Bulanan' if dur == '30 hari' else 'Selamanya'}\n"
                    f"Sisa kuota: {10 - promo_count()}",
                    parse_mode="Markdown"
                )
                await update.message.reply_text("✅ Pembayaran valid! Akses premium aktif.")
            else:
                await update.message.reply_text(f"❌ Nominal tidak sesuai. Terbaca Rp{nominal_max:,}, seharusnya Rp{harga_harapan:,}.")
        else:
            await update.message.reply_text("❌ Tidak bisa membaca nominal, coba foto lebih jelas.")
    except Exception as e:
...
async def price_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if raw.upper() in ("XAUUSD", "XAUU", "GOLD", "EMAS"):
        ...
        await update.message.reply_text(f"📈 Harga {raw.upper()} saat ini: {last_price:.6f}")
...
async def status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row and row[0]:
        try:
            ...
            if sisa > 365*10:
                txt = "✅ Status: Premium Selamanya (Lifetime)"
            elif sisa > 0:
                txt = f"✅ Status: Premium\n⏳ Sisa: {sisa} hari (sampai {row[0]})"
            else:
                txt = "❌ Masa premium telah habis."
        except:
            txt = "✅ Status: Premium"
    else:
